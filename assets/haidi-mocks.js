...
(function () {
  var T = '#47B9BB', C = '#F08989', M = '#AAB2BF', F = '#F4F6FA';
    ...
    return '<div style="display:grid;grid-template-columns:repeat(' + items.length + ',1fr);gap:12px">' +
      items.map(function (k) {
        return '<div style="background:rgba(255,255,255,0.025);border:1px solid var(--line);border-radius:11px;padding:14px">' +
          '<div style="font-size:11px;color:#7E8794;text-transform:uppercase;letter-spacing:.07em">' + k.label + '</div>' +
  ...
  function table(head, rows) {
    ...
    return '<div style="border:1px solid var(--line);border-radius:11px;overflow:hidden">' +
      '<div style="display:grid;grid-template-columns:' + cols + ';background:rgba(71,185,187,0.10);padding:10px 14px;font-size:11px;letter-spacing:.05em;text-transform:uppercase;color:#9FB6B7">' +
      head.map(function (h) { return '<span>' + h + '</span>'; }).join('') + '</div>' +
      rows.map(function (r, i) {
        return '<div style="display:grid;grid-template-columns:' + cols + ';padding:11px 14px;font-size:13px;color:#C7CFDA;border-top:1px solid var(--line)' + (i % 2 ? ';background:rgba(255,255,255,0.015)' : '') + '">' +
  ...
  function shell(title, tabs, body, active) {
    ...
    var tabsHtml = (tabs || []).map(function (t, i) {
      return '<span style="font-size:12px;padding:6px 12px;border-radius:7px;' + (i === 0 ? 'background:rgba(71,185,187,0.16);color:' + T + ';border:1px solid rgba(71,185,187,0.3)' : 'color:#7E8794') + '">' + t + '</span>';
    }).join('');
  ...
  function statusRow(label, status, tone) {
    var col = tone === 'c' ? C : tone === 'm' ? '#7E8794' : T;
  ...
  var BUILDERS = {
    ...
    drivers: function () {
      return shell('Demand Drivers', ['Ranked', 'Seasonal', 'Events'],
        '<div style="font-size:12px;color:#7E8794">Impact on the selected forecast</div>' +
      ...
      return '<div style="display:flex;min-height:340px">' +
        ...
        '<div style="flex:1;display:flex;flex-direction:column">' +
          ...
          '<div style="padding:16px;display:flex;flex-direction:column;gap:12px;flex:1">' +
            '<div style="align-self:flex-end;max-width:82%;background:rgba(71,185,187,0.14);border:1px solid rgba(71,185,187,0.25);border-radius:14px 14px 4px 14px;padding:10px 13px;font-size:13px;color:#E7EEF0">What changed in this forecast?</div>' +
            '<div style="align-self:flex-start;max-width:90%;background:rgba(255,255,255,0.04);border:1px solid var(--line);border-radius:14px 14px 14px 4px;padding:10px 13px;font-size:13px;color:#C7CFDA;line-height:1.5">The promotion calendar driver added +18%, partly offset by a −6% competitor signal. Seasonality is in line with prior periods.</div>' +
            '<div style="margin-top:auto;border:1px solid var(--line);border-radius:10px;padding:10px 13px;font-size:12px;color:#7E8794">Ask about drivers, scenarios, exceptions…</div>' +
          '</div>' +
        '</div></div>';
    },
    data: function () {
    ...
    import: function () {
      return shell('Data Import', ['Upload', 'Map', 'Validate'],
        '<div style="border:1px dashed rgba(71,185,187,0.4);border-radius:11px;padding:20px;text-align:center;font-size:13px;color:#9FB6B7;background:rgba(71,185,187,0.05)">demand_history_2026.csv · 14,820 rows</div>' +
  ...
  var FRAGMENTS = {
    ...
    'data-import': function () { return frag('Data Import', '<div style="border:1px dashed rgba(71,185,187,0.4);border-radius:9px;padding:14px;text-align:center;font-size:12px;color:#9FB6B7;background:rgba(71,185,187,0.05);margin-bottom:12px">demand_history.csv</div>' + bars([{label:'Mapped', val:'12/12', pct:100},{label:'Valid', val:'98%', pct:98}])); },
    ...
    'settings': function () {
      var toggle = function (l, on) { return '<div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;font-size:13px;color:#C7CFDA"><span>' + l + '</span><span style="width:34px;height:19px;border-radius:999px;background:' + (on ? T : 'rgba(255,255,255,0.12)') + ';position:relative"><span style="position:absolute;top:2px;' + (on ? 'right:2px' : 'left:2px') + ';width:15px;height:15px;border-radius:50%;background:#fff"></span></span></div>'; };
